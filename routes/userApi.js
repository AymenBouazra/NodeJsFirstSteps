...
// 1- GET ALL TODOS
router.get('/crud',passport.authenticate('bearer',{session:false}), async (req, res) => {
  const crudTodo = await crud.find().populate('crud')
  res.json(crudTodo);
});

// 2- GET TODOS BY ID 
router.get('/crud/:id',passport.authenticate('bearer',{session:false}), async (req, res) => {
  const findUser = await crud.findById(req.params.id)
  res.json(findUser);
});
// filter by age greater than 20
router.get('/crud/filter/byage',passport.authenticate('bearer',{session:false}), async (req, res) => {
...
router.get('/crud/filter/or',passport.authenticate('bearer',{session:false}), async (req, res) => {
  const findUser = await crud.find().or([{ age: 26 }, { age: 19 }])
  res.json(findUser);
});
// filter with or query
router.get('/crud/filter/and',passport.authenticate('bearer',{session:false}), async (req, res) => {
...
// 3- ADD TODO
router.post('/crud',passport.authenticate('bearer',{session:false}), async (req, res) => {
  const createdUser = await crud.create(req.body);
  res.json({ createdUser });
});

// 4- UPDATE TODO BY ID
router.put('/crud/:id',passport.authenticate('bearer',{session:false}), async (req, res) => {
  const updateUser = await crud.findByIdAndUpdate(req.params.id, { firstName: 'Nadhem' })
  res.json(updateUser);
});
// 5- DELETE TODO BY ID
router.delete('/crud/:id',passport.authenticate('bearer',{session:false}), async (req, res) => {
  const deleteUser = await crud.findByIdAndDelete(req.params.id)
  res.json(deleteUser);
});

router.put('/crud/affectTodo/:idTodo/:idUser',passport.authenticate('bearer',{session:false}), async (req, res) => {
...
router.put('/crud/disaffectTodo/:idTodo/:idUser',passport.authenticate('bearer',{session:false}), async (req, res) => {
